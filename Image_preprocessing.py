...
import pandas as pd
from imagedehazer import image_dehazer

class ImageDehazer():
    def __init__(self):
        self.image_Dehazer = image_dehazer()
    ...
    def dehazer(self,image):
        print(image.shape)
        HazeImg = image
        HazeCorrectedImg, HazeMap = self.image_Dehazer.remove_haze(HazeImg)	
...
class ImageDerained():
    ...
    def guided_filter(self,I, p, r, eps):
        ...
        I = I.astype(np.float32) / 255.0 # guide 
        p = p.astype(np.float32) / 255.0 # input image
