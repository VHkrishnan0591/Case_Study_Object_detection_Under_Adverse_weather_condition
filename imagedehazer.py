...
class image_dehazer():
    ...
    def __AirlightEstimation(self, HazeImg):
        if (len(HazeImg.shape) == 3):
            for ch in range(len(HazeImg.shape)):
                ...
                minImg = cv2.erode(HazeImg[:, :, ch], kernel) #reduces the intensity of pixels 
                self._A.append(int(minImg.max())) # maximum pixel value
        else:
            kernel = np.ones((self.airlightEstimation_windowSze, self.airlightEstimation_windowSze), np.uint8)
    ...
    def __BoundCon(self, HazeImg):
        ...
        self._Transmission = cv2.morphologyEx(self._Transmission, cv2.MORPH_CLOSE, kernel=kernel) #smooth it and fill small holes
    ...
    def __CalTransmission(self, HazeImg):
        ...
        KirschFilters = self.__LoadFilterBank() #Kirsch edge detection filters,
    ...
    def __removeHaze(self, HazeImg):
        ...
        epsilon = 0.0001
        Transmission = pow(np.maximum(abs(self._Transmission), epsilon), self.delta) #avoiding division by zero.
    ...
    def remove_haze(self, HazeImg):
        self.__AirlightEstimation(HazeImg) # estimate the airlight
        self.__BoundCon(HazeImg) # create the transmission map
        self.__CalTransmission(HazeImg)  #estimating the transmission map 
